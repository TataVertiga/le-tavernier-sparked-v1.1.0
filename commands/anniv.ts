import { SlashCommandBuilder,
  ...
  GuildMember, MessageFlags } from "discord.js";
    ...
    if (sub === "set") {
      ...
      if (!validDate(raw)) {
        return interaction.reply({
          content:
            "❌ Format incorrect, gueux ! Utilise `JJ-MM` ou `JJ-MM-AAAA`.\nExemple : `04-11-1987`",
      ...
      for (const [uid, entry] of Object.entries(birthdays)) {
        ...
        if (guild) {
          try {
            ...
            if (member) displayName = (member as GuildMember).displayName || member.user.username;
          } catch {
            // ignore
          }
        }
        ...
        if ((entry as any).year && !isNaN((entry as any).year)) {
