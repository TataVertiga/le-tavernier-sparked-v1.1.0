...
function makeEmbed(title: string, emojiTitre: string, items: RoleItem[]) {
  const desc = items
  ...
  return new EmbedBuilder()
    ...
    .setDescription(
      `Réagis avec les émojis pour obtenir ou retirer un rôle.\n\n${desc}\n\n` +
...
async function addReactionsSequential(message: any, emojis: string[]) {
...
export async function publishRolePanels(target: TextChannel) {
  const cfg = loadConfig();
  const mapPartial: Record<string, { emoji: string; roleId: string }[]> = {};
  const panels = [
...
export default {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    const target = (interaction.options.getChannel("salon") || interaction.channel) as TextChannel | null;
    ...
    await interaction.editReply(
      `✅ Panneaux publiés dans <#${target.id}> et mappage **fusionné** dans \`data/reaction-map.json\`.`
    );
  }
};
