...
import { normalizeClipThumbnail, type TwitchClip } from "../services/twitchClips.js";
...
async function getBroadcasterId(login: string, appToken: string): Promise<string> {
  const clientId = process.env.TWITCH_CLIENT_ID!;
  ...
  if (!res.ok) throw new Error(`users?login=… HTTP ${res.status}`);
...
async function fetchLatestClip(hours = 6): Promise<TwitchClip | null> {
  const login = process.env.TWITCH_USERNAME;
  if (!login) throw new Error('TWITCH_USERNAME manquant dans .env');
  ...
  if (!res.ok) throw new Error(`helix/clips HTTP ${res.status}`);
...
function clipToEmbed(clip: TwitchClip): EmbedBuilder {
  ...
  return new EmbedBuilder()
    ...
    .addFields(
      { name: '👀 Vues', value: String(clip.view_count ?? 0), inline: true },
      { name: '⏱️ Durée', value: `${clip.duration?.toFixed(1) ?? '?'}s`, inline: true },
    )
