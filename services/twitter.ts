...
function describeTwitterError(error: unknown): string {
  const value = error as {
    code?: number;
    message?: string;
    data?: { title?: string; detail?: string; type?: string; errors?: Array<{ message?: string }> };
  };
...
export type TwitterPublishResult = { status: 'posted'; postId?: string } | { status: 'duplicate' };

export async function publierTweetLiveTwitch(
  twitchUser: string,
  liveId: string,
