...
export type GiveawayEntry = {
  ...
  participants: string[];   // IDs utilisateurs
  description: string;
};

export type GiveawayData = Record<string, GiveawayEntry>; // par ID de giveaway
...
export function loadGiveaways(): GiveawayData {
  ...
  try {
    ...
    if (parsed && typeof parsed === "object" && !Array.isArray(parsed)) return parsed as GiveawayData;
...
export function saveGiveaways(data: GiveawayData, createBackup = false) {
  if (!fs.existsSync(DATA_DIR)) fs.mkdirSync(DATA_DIR, { recursive: true });
  if (createBackup) backupGiveawayFile();
