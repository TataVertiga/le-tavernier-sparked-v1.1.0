...
function firstValue(names: string[], fallback = ""): string {
...
export const discordIds = Object.freeze({
  ...
  giveawayChannel: firstValue(
    ["GIVEAWAY_CHANNEL_ID", "GIVEAWAYS_CHANNEL_ID"],
    "1123654628681199677",
  ),
...
const allPresent = (names: string[]) => names.every(name => Boolean(process.env[name]?.trim()));

const definitions: Record<ServiceKey, ServiceDefinition> = {
  discord: {
    label: "Discord",
...
export type RuntimeServiceState = "waiting" | "running" | "error";
const runtimeStates = new Map<ServiceKey, { state: RuntimeServiceState; detail?: string }>();
...
export function serviceReadiness(key: ServiceKey) {
...
export function markService(key: ServiceKey, state: RuntimeServiceState, detail?: string) {
  runtimeStates.set(key, { state, detail });
}

export function serviceStatusLines(): string[] {
  return (Object.keys(definitions) as ServiceKey[]).map(key => {
    const service = serviceReadiness(key);
    if (!service.enabled) return `• ${service.label}: ⚫ désactivé`;
    if (service.missing.length) return `• ${service.label}: 🟠 configuration incomplète`;
    if (service.runtime?.state === "error") return `• ${service.label}: 🔴 erreur`;
    if (service.runtime?.state === "running") return `• ${service.label}: 🟢 actif`;
