import { EmbedBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle, time, TimestampStyles, } from "discord.js";
/* ===============================================================
   BRAND / CONSTANTES
   =============================================================== */
const BRAND = {
...
export function createClipEmbed(args) {
    const { title, url, thumbnailUrl, broadcasterName, creatorName, viewCount, createdAtISO, } = args;
    ...
    const embed = new EmbedBuilder()
        ...
        .join("\n"))
        .setFooter({
        text: `👀 ${formatViews(viewCount)} vues` +
...
export function buildTwitchLiveMessage(params) {
    const { channelUrl, streamTitle, gameName, thumbnail_template_url, started_at, displayName, isMature, rolePingId, } = params;
    ...
    const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setLabel("📺 Voir le live").setStyle(ButtonStyle.Link).setURL(channelUrl));
    return {
        content,
        embeds: [embed],
        components: [row],
...
export function buildTwitchClipMessage(clip) {
    ...
    const embed = new EmbedBuilder()
        ...
        .join("\n"))
        .setFooter({
        text: `👀 ${formatViews(clip.view_count)}` +
            (created ? ` • ${time(created, TimestampStyles.RelativeTime)}` : ""),
        iconURL: BRAND.twitchIcon,
    });
    if (clip.thumbnail_url)
        embed.setImage(clip.thumbnail_url);
    const cta = new ActionRowBuilder().addComponents(new ButtonBuilder().setLabel("🎬 Voir le clip").setStyle(ButtonStyle.Link).setURL(clip.url));
...
export function buildYouTubeVideoMessage(video) {
    ...
    const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setLabel("▶️ Voir la vidéo").setStyle(ButtonStyle.Link).setURL(video.url));
    return {
        content,
        embeds: [embed],
        components: [row],
...
export function buildTikTokButtons(videoUrl, authorName, authorUrl) {
