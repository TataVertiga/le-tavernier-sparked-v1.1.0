...
import { publishLivePost, socialPlatforms, } from "../services/socialPublisher.js";
...
export default {
    data: new SlashCommandBuilder()
        ...
        .setRequired(false)
        .addChoices({ name: "X / Twitter", value: "twitter" }, { name: "Facebook", value: "facebook" }, { name: "Bluesky", value: "bluesky" }, { name: "Threads", value: "threads" }, { name: "Toutes", value: "all" }))
    ...
    async execute(interaction) {
        ...
        const platforms = selected === "all"
            ? socialPlatforms
            : [selected];
        const reports = await Promise.all(platforms.map(platform => publishLivePost(platform, user, liveId)));
        const icons = { twitter: "🐦", facebook: "👤", bluesky: "🦋", threads: "🧵" };
