...
async function fetchDetails(videoId) {
    ...
    const bestThumb = info.snippet?.thumbnails?.maxres?.url ||
...
export async function checkYoutube(client) {
    if (!DISCORD_CHANNEL_ID || !YT_RSS_URL)
        return;
    ...
    try {
        ...
        for (const it of items) {
            ...
            if (lastDate && published <= lastDate)
                continue;
            // Anti-repost: si déjà annoncé (mémoire IDs)
            if ((lastData.lastIds || []).includes(videoId))
                continue;
