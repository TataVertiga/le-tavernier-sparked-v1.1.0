...
let cache = { lastDate: dayjs().tz(TZ).format("YYYY-MM-DD"), sent: [] };
function loadCache() {
    try {
...
// --- Répliques ---
const REPLIQUES_AGE = [
    (userId, age) => `🍻 Bon anniversaire <@${userId}> ! Encore un an de plus au compteur, ça commence à sentir la poussière (tu as maintenant ${age} ans) !`,
    (userId, age) => `🎉 Santé <@${userId}> ! ${age} ans aujourd’hui… On ne dirait pas, et pourtant la bière ne ment jamais.`,
    (userId, age) => `🥳 Joyeux anniversaire <@${userId}> ! ${age} ans, et pas une ride (enfin presque). Passe boire ta pinte !`,
    (userId, age) => `🍰 Un an de plus pour <@${userId}>, ${age} ans, t’approches de la catégorie vétéran de la Taverne !`,
    (userId, age) => `🎂 <@${userId}>, ${age} ans aujourd’hui ! Tu gagnes le droit d’offrir ta tournée à tous les gueux présents.`
];
const REPLIQUES_SANS_AGE = [
    (userId) => `🍻 Bon anniversaire <@${userId}> ! L’âge, c’est dans la tête (et parfois dans le foie).`,
    (userId) => `🎉 <@${userId}>, la Taverne te souhaite un anniversaire mystérieux, comme ton âge.`,
    (userId) => `🥳 On ne connaît pas ton âge, mais joyeux anniversaire quand même <@${userId}> ! Profite bien !`,
    (userId) => `🍰 Bon anniversaire à notre gueux anonyme préféré, <@${userId}> !`,
    (userId) => `🎂 Tu refuses de donner ton âge, <@${userId}> ? Pas grave, t’as quand même le droit à une pinte offerte !`
];
// --- Date parser ---
function parseDateSmart(dateString) {
    ...
    if (parts.length === 2) {
        ...
        if (B > 12)
            return { day: b.padStart(2, "0"), month: a.padStart(2, "0") };
        return { day: a.padStart(2, "0"), month: b.padStart(2, "0") };
    }
    if (parts.length === 3) {
        let [a, b, c] = parts;
        if (c.length === 4) {
            const A = parseInt(a, 10), B = parseInt(b, 10);
            if ([A, B, parseInt(c, 10)].some(Number.isNaN))
                return null;
            if (A > 12)
                return { day: a.padStart(2, "0"), month: b.padStart(2, "0"), year: c };
            if (B > 12)
                return { day: b.padStart(2, "0"), month: a.padStart(2, "0"), year: c };
            return { day: a.padStart(2, "0"), month: b.padStart(2, "0"), year: c };
        }
    }
    return null;
...
// --- Récupération salon ---
async function getAnnivChannel(client) {
    if (!ANNIV_CHANNEL_ID)
        return null;
...
// --- Vérif & envoi ---
async function checkAndSendAnniversaires(client) {
    if (isPublishingPaused()) {
        console.log("[ANNIV] Publication bloquée (pause=ON)");
        return;
    }
    ...
    for (const anniv of anniversaires) {
        ...
        if (dateFormatee !== todayShort)
            continue;
        if (cache.sent.includes(anniv.userId))
            continue;
        let age;
...
// --- Init ---
export function initAnniversaires(client) {
    loadCache();
    if (!ANNIV_CHANNEL_ID)
        console.warn("[ANNIV] ⚠️ ANNIV_CHANNEL_ID manquant !");
    if (!GOOGLE_SHEET_ID)
        console.warn("[ANNIV] ⚠️ GOOGLE_SHEET_ID manquant !");
