...
import { twitchChannelUrl } from "./twitch.js";
function randomRpMessage(roleId) {
...
async function sendDiscordAnnouncement(client, stream) {
    ...
    const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setLabel("📺 Voir le live").setURL(url).setStyle(ButtonStyle.Link));
...
export async function announceTwitchLive(client, stream) {
...
export function announceTwitchEnd() {
