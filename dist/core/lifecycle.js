...
import { serviceReadiness, markService } from "./config.js";
...
async function startIsolatedService(key, start) {
...
async function startExternalServices(client) {
    await Promise.all([
        ...
        startIsolatedService("twitch", () => {
            void startTwitchWatcher({
                ...
                onError: error => markService("twitch", "error", error instanceof Error ? error.message : String(error)),
            }).catch(error => {
        ...
        startIsolatedService("clips", () => {
            void startTwitchClipsWatcher({
                ...
                onError: error => markService("clips", "error", error instanceof Error ? error.message : String(error)),
            }).catch(error => {
...
export function registerLifecycle(client) {
